...
export const handleCreateLine: RequestHandler = async (req, res) => {
  try {
    ...
    if (!decoded.teamId)
      return res.status(400).json({ error: "Team ID missing in token" });
    ...
    const distributedDoc = await NumberLine.findOne({
      teamId: decoded.teamId,
      status: "distributed",
      content: {
        $regex: `^${sanitizedContent.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`,
        $options: "i",
      },
    });
    if (distributedDoc) {
    ...
    res
      .status(500)
      .json({ error: (error as Error).message || "Failed to create line" });
  }
};
...
export const handleCreateLines: RequestHandler = async (req, res) => {
  try {
    ...
    if (sanitized.length === 0)
      return res.status(400).json({ error: "No valid contents provided" });

    // remove any contents already present in distributed lines for this team
    const distributedDocs = await NumberLine.find({
      teamId: decoded.teamId,
      status: "distributed",
    });
    const distributedSet = new Set(
      distributedDocs.map((d) =>
        (d.content || "").toString().trim().toLowerCase(),
      ),
    );

    const filtered = sanitized.filter(
      (s: string) => !distributedSet.has(s.toString().trim().toLowerCase()),
    );

    // compute skipped (duplicates removed because present in distributed)
    const skipped = sanitized.filter((s: string) =>
      distributedSet.has(s.toString().trim().toLowerCase()),
    );
    ...
    const createdLines =
      newLines.length > 0
        ? await NumberLine.insertMany(newLines, { ordered: false })
        : [];
    const createdDuplicates =
      duplicateDocs.length > 0
        ? await NumberLine.insertMany(duplicateDocs, { ordered: false })
        : [];
    ...
    try {
      ...
      if (io) {
        if (createdLines.length > 0) {
          io.to(`team_${decoded.teamId}`).emit("lines_added", {
            lines: createdLines,
          });
        }
        if (createdDuplicates.length > 0) {
          io.to(`team_${decoded.teamId}`).emit("duplicates_added", {
    ...
    res
      .status(500)
      .json({ error: (error as Error).message || "Failed to create lines" });
  }
};
...
export const handleMoveToQueue: RequestHandler = async (req, res) => {
  try {
    ...
    try {
      const io = req.app.get("io");
      if (io) {
        io.to(`team_${decoded.teamId}`).emit("lines_moved_to_queue", {
          lineIds,
        });
      }
    } catch (e) {
...
export const handleMoveToDistributor: RequestHandler = async (req, res) => {
  try {
    ...
    try {
      const io = req.app.get("io");
      if (io) {
        io.to(`team_${decoded.teamId}`).emit("lines_moved_to_distributor", {
          lineIds,
        });
      }
    } catch (e) {
      console.error("Emit lines_moved_to_distributor failed", e);
    }
...
export const handleClearDistributor: RequestHandler = async (req, res) => {
  try {
    ...
    const result = await NumberLine.deleteMany({
      teamId: decoded.teamId,
      status: "distributed",
    });
    // emit update
    try {
      const io = req.app.get("io");
      if (io) {
        io.to(`team_${decoded.teamId}`).emit("distributor_cleared", {
    ...
    res.json({
      message: "Cleared distributor assignments",
      deletedCount: result.deletedCount,
    });
  } catch (error) {
